...
public class TPCH {
	...
	public static void tpch(List<Server> servers, boolean load) throws Exception {
		...
		System.out.println("Explain queries?[y|n]");
					...
					if (queries.size() == 3) {
						...
						for (int i = 0; i < queries.size(); i++) {
							...
							if (q.toLowerCase().contains("view")) {
								...
								rs.setAnalyzeAndRewriteTime(
										rs.getAnalyzeAndRewriteTime() + rs2.getAnalyzeAndRewriteTime());
					...
					if (queries.size() == 1) {
						...
						if (timeout > 0) {
							try {
								connection.setAutoCommit(false);
								statement.executeUpdate("SET LOCAL statement_timeout=" + timeout + ";");
								rs2 = statement.executeQuery(explain + q + ";");
								connection.commit();
							} catch (Exception e) {
								System.out.println(e.getMessage());
								connection.close();
					...
					if (queries.size() == 3) {
						...
						for (int i = 0; i < queries.size(); i++) {
							...
							if (q.toLowerCase().contains("view")) {
								...
								tmptime = System.currentTimeMillis();
								rs2 = owner.executeQuery(explain + q + ";", false);
								time3 += System.currentTimeMillis() - tmptime;
							}
						}
						while (rs2.next()) {
